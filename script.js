...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            ...
            // Fetch the metadata (info.json) for each folder
            try {
                let metadataResponse = await fetch(`$/info.json`);
                let metadata = await metadataResponse.json(); // Parse JSON response
                ...
                // console.error(`Failed to fetch info.json or cover.jpg for folder ${folder}:`, error);
            }
        }
    }
